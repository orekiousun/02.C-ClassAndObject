...
/*
    ...
    纯虚析构语法：
        virtual ~类名() = 0;
        类名::~类命(){纯虚析构函数定义}   // 纯虚析构函数也需要代码实现，因为父类也有可能会有属性开辟到堆区，需要手动释放
...
void test02()
{
    ...
    // 每加virtual关键字时，输出为：1，非静态成员函数和成员变量分开存储，而空对象（类）占用的空间大小为1
    ...
    // 当没有发生重写时，子类从父类中继承指针vfptr，该指针指向的虚函数表中记录的是Animal中的函数地址即：&Animal::speak
